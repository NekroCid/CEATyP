...
class Index(Handler):
    #Metodo get
    def get(self):
        contrasena = SHA256.new('ceatyp').hexdigest()
        logging.info('Hash:'+ str(contrasena))
...
class Perfil(Handler):
    ...
    def get(self):
        mail = self.session.get('Correo')
        self.render("perfil.html", correo=mail)
...
class Calendario(Handler):
    ...
    def get(self):
        ...
        # self.render("calendario.html", correo=mail,response_calendar=summary)
...
class CalendarioAdd(Handler):
    ...
    def post(self):
        ...
        fechaini = fechaini+'T00:00:00-06:00'
        fechafin = self.request.get('fechafin')
        fechafin = fechafin+'T00:00:00-06:00'
        event = {
            'summary': titulo,
            'location': locacion,
...
class CuentaNew(Handler):
	...
	def post(self):
		...
		if control_user == control:
			msg = "Gracias por registrarse..."
			self.render("addcuenta.html", msg=msg)
